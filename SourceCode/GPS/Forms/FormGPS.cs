...
/*A ordem é:
 *
 * O temporizador do watchdog expira e executa esta função tmrWatchdog_tick().
 * 50 vezes por segundo, então statusUpdateCounter conta até 25 e atualiza o menu de faixas etc. em 2 hz
 * também garante que haja novas frases aparecendo, caso contrário, mostra **** No GGA ....
 * saveCounter marca 2 x por segundo, usado no final da rotina de sorteio a cada minuto para salvar um backup do campo
 * então a função ScanForNMEA verifica se há uma frase completa se contida em pn.rawbuffer
 * caso contrário, ele volta e espera o próximo gatilho do watchdog e começa tudo de novo
 * se houver uma nova frase, UpdateFix() é chamado
 * Imediatamente CalculateLookAhead(), sem saltos, é chamado para determinar as antecipações e disparar distâncias para salvar triângulos plotados
 * Então UpdateFix() continua.
 * Engate, pivô, localizações de antenas, etc. e direções são descobertas se trigDistance for acionado
 * Quando isso é feito, DoRender() é chamado na tela visível do OpenGL e sua rotina de desenho _draw é executada
 * antes que os triângulos sejam desenhados, frustum cull calcula quantos dos triângulos devem ser desenhados
 * Quando estiver completo, ele aciona o Draw de controle de seção, seu frustum cull e determina se as seções devem estar ativadas
 * ProcessSectionOnOffRequests() é executado e isso faz a seção off magic
 * SectionControlToArduino() executa e cospe o controle da máquina da porta com base nas seções ativadas ou desativadas
 * Se o campo precisar ser salvo (1,5 minuto desde a última vez), o campo será salvo
 * Agora o programa está "Concluído" e aguarda o próximo acionador do watchdog, determina se uma nova frase está disponível etc.
 * e começa tudo de cima.
 */